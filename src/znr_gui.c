...
struct znr_gui {
	...
	unsigned long long	visible_bgs_no;
...
#define ZNR_TYPE_BLOCKGROUP_ITEM	znr_gui_bg_item_get_type()
G_DECLARE_FINAL_TYPE(znr_gui_bg_item, znr_gui_bg_item,
		     ZNR, BLOCKGROUP_ITEM, GObject)

struct _znr_gui_bg_item {
	GObject parent;
	unsigned int bg_no;
	struct znr_blockgroup *bg;
};

G_DEFINE_TYPE(znr_gui_bg_item, znr_gui_bg_item, G_TYPE_OBJECT)

static GtkWidget *znr_gui_create_grid(void);

static void znr_gui_bg_item_init(struct _znr_gui_bg_item *item)
{
	item->bg_no = 0;
	item->bg = NULL;
}
...
/*
 * Note: znr_gui_bg_itemClass is generated by the macro above, hence
 * we can't avoid the camelcase.
 */
static void znr_gui_bg_item_class_init(znr_gui_bg_itemClass *class) {}

static struct _znr_gui_bg_item *znr_gui_bg_item_new(unsigned int bg_no,
						    struct znr_blockgroup *bg)
{
	struct _znr_gui_bg_item *item =
...
static int znr_gui_report_bgs(unsigned int bg_start, unsigned int nr_bgs)
{
	int ret;
 ...
 * blockreport.
 */
static gboolean znr_gui_deferred_report_bgs(gpointer data)
{
	struct znr_gui_bg_report_info *bg_rep = data;
	unsigned int start = bg_rep->starting_bg;
	unsigned int nr_bgs = bg_rep->nr_bgs;
	int ret;

	ret = znr_gui_report_bgs(start, nr_bgs);
	if (ret) {
...
{
	long long w = width * write_pointer / max_nr_sectors;

	if (w > width)
		w = width;

	znr_gui_draw_rect(cr, color, x, 0, w, height);
	return w;
}
				    ...
				    const GdkRGBA *written)
{
	...
	if (!(bg->flags & ZNR_BG_HAS_FS_WP &&
	      bg->flags & ZNR_BG_HAS_DEV_ZONE_WP))
		return;

	wp_delta = bg->fs_wp - bg->dev_zone_wp;
	if (!wp_delta)
		return;

	znr_gui_draw_wp_rect(cr, &znrg.color_delta, x, 0, width, height,
			     wp_delta, bg->nr_sectors);
}

static void znr_gui_draw_bg_num(struct znr_gui_blockgroup *bg,
				cairo_t *cr, int width, int height)
{
...
static bool znr_gui_bg_tab_open(struct znr_gui_blockgroup *bg,
...
static bool znr_gui_should_draw_bg_extents(struct znr_gui_blockgroup *bg,
					   struct znr_gui_extents_tab **ext_tab)
{
	 ...
	 * blockgroup tabs
	 */
	if (znr_gui_bg_tab_open(bg, ext_tab))
		return true;
...
static void
znr_gui_draw_bg_extents(struct znr_gui_blockgroup *gui_bg, cairo_t *cr,
			int width, int height)
{
	...
	if (!znr_gui_should_draw_bg_extents(gui_bg, &tab))
		return;
	...
	for (i = 0; i < tab->nr_extents; i++) {
		ext = &tab->extents[i];
		if (!znr_gui_extent_is_in_bg(ext, bg))
			continue;
...
static void znr_gui_bg_draw_cb(GtkDrawingArea *drawing_area,
			       cairo_t *cr, int width, int height,
			       gpointer user_data)
{
...
static void znr_gui_click_bg_cb(GtkGestureClick *self, gint n_press,
				gdouble x, gdouble y, gpointer user_data)
{
	...
	ret = znr_gui_report_bgs(gui_bg->bg_no, 1);
	if (ret) {
		znr_gui_err("Report Blockgroup Failed",
				   ...
				   gpointer user_data  __attribute__((unused)))
{
	...
	znr_gui_draw_legend("Sequential (free)",
	 ...
	 */
	if (znr.mnt_dir.fs && znr.mnt_dir.fs->type == ZNR_FS_BTRFS)
	...
	/*
	 * Difference of the filesystem allocated and the device
	 * zone write pointer.
	 */
	znr_gui_draw_legend("Unwritten (allocated, not written)",
			    &znrg.color_delta, cr, &x, y, widget);
}

static void znr_gui_bg_da_size(int *width, int *height)
{
...
static int znr_gui_get_first_bg_in_view(unsigned int *first_bg)
{
	GtkAdjustment *vadj;
	int first_row;
	int bgs_padding;
...
static gboolean znr_gui_refresh_local_cb(gpointer user_data)
{
	...
	if (znr_gui_get_first_bg_in_view(&first_bg))
		znr_gui_err("Failed to refresh local blockgroups\n", NULL);

	if (znrg.visible_bgs_no > znr.nr_bgs) {
	...
	znr_gui_close_extents_dialog();
	znr_gui_report_bgs(first_bg, znrg.visible_bgs_no);
	znr_gui_update();
...
static void znr_gui_refresh_cb(GtkWidget *widget __attribute__((unused)),
			       gpointer user_data __attribute__((unused)))
{
	struct znr_gui_bg_report_info *bg_rep;

	bg_rep = calloc(1, sizeof(struct znr_gui_bg_report_info));
        if (!bg_rep) {
	...
	g_idle_add(znr_gui_deferred_report_bgs, bg_rep);
}

static void znr_gui_show_bg_cb(GtkWidget *button __attribute__((unused)),
			       gpointer user_data __attribute__((unused)))
{
	const char *text;
	char *endptr;
	...
	text = gtk_editable_get_text(GTK_EDITABLE(znrg.show_bg_entry));
	if (!text || !strlen(text))
		return;
				 ...
				 GtkListItem *list_item, gpointer user_data)
{
	...
	struct _znr_gui_bg_item *bg_item;
	GtkWidget *da;
	...
	if (!gui_bg) {
		...
		g_signal_connect(ctrl, "enter",
				 G_CALLBACK(znr_gui_bg_enter_cb),
				 gui_bg);
		g_signal_connect(ctrl, "leave",
				 G_CALLBACK(znr_gui_bg_leave_cb),
				 gui_bg);
		...
		g_signal_connect(gesture, "pressed",
				 G_CALLBACK(znr_gui_click_bg_cb),
				 gui_bg);
	}
...
static GtkWidget *znr_gui_create_grid(void)
{
	...
	if (znrg.drawing_areas) {
		...
		znrg.visible_bgs_no = 0;
	}
	znrg.drawing_areas = g_hash_table_new(g_direct_hash, g_direct_equal);

	/* Create a blockgroup g_list */
	znrg.bg_list = g_list_store_new(ZNR_TYPE_BLOCKGROUP_ITEM);
	for (i = 0; i < znr.nr_bgs; ++i) {
		bg_item = znr_gui_bg_item_new(i, &znr.bgs[i]);
		g_list_store_append(znrg.bg_list, bg_item);
		g_object_unref(bg_item);
	}

	/* Use no_selection otherwise we see blockgroup 0 selected on start */
	selection = GTK_SELECTION_MODEL(gtk_no_selection_new(G_LIST_MODEL(znrg.bg_list)));
...
static void window_size_changed_cb(GtkWidget *widget)
{
	...
	znr_gui_bg_da_size(&bg_da_w, NULL);
	if (!bg_da_w || !w)
		return;
	/*
...
static void znr_gui_create_app(GtkApplication *app, gpointer user_data)
{
	...
	g_signal_connect(entry, "activate",
			 G_CALLBACK(znr_gui_show_bg_cb), NULL);
	znrg.show_bg_entry = entry;
